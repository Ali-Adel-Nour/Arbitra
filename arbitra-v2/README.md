...
### 4. Persistent Hardware Passkey Storage (Prisma ORM)
We integrated **Prisma ORM with SQLite** for local development (which can be instantly swapped to PostgreSQL for production). The backend uses strict relational tables (`Admin` and `Authenticator`) to permanently and securely store the P256 public keys generated by the hardware enclave. This ensures mathematical chain-of-custody and full crash-recoverability across server reboots.
